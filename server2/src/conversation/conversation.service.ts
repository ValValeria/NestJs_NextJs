...
export class ConversationService {
  ...
  async getMessages(receiver: User, sender: User): Promise<Conversation[]> {
    return this.conversationModel.findAll({
      where: {
        $or: [
          { userId: receiver.id, receiverId: sender.id },
          { userId: sender.id, receiverId: receiver.id },
        ],
      },
    });
  }

  async getMessagesByUser(user: User): Promise<Conversation[]> {
    return this.conversationModel.findAll({
      where: {
        $or: [{ userId: user.id }, { receiverId: user.id }],
      },
    });
  }
}
